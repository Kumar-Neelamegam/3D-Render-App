...
public class AppUtils {
  ...
  static File getWorkingDirectory(Activity activity) {
    File directory =
        new File(Environment.getExternalStorageDirectory(), activity.getApplicationContext().getString(R.string.app_name)+"-img");
    if (!directory.exists()) {
      directory.mkdir();
    }
    return directory;
  ...
  static FileUri createImageFile(Activity activity, String prefix) {
    ...
    File image = null;
    try {
      image = File.createTempFile(prefix + String.valueOf(System.currentTimeMillis()), ".png",getWorkingDirectory(activity));
    } catch (IOException e) {
      e.printStackTrace();
    }
    if (image != null) {
